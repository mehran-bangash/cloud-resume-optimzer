    ...
    <>
      <div className="border-b border-slate-800 px-4 md:px-8 py-3 flex items-center justify-between bg-slate-950 sticky top-0 z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/tracker"
            className="text-xs text-slate-500 hover:text-teal-400 transition-colors px-2 py-1 rounded-lg hover:bg-slate-800 flex items-center gap-1"
          >
            📋 Tracker
          </Link>
          ...
          {saveStatus ? (
            <div className={`flex items-center gap-1.5 text-xs px-2 py-1 rounded-full transition-all ${saveStatus === "saving"
          ...
          <div className="hidden sm:flex gap-1 mr-2">
            {TEMPLATES.map((t) => (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg capitalize font-medium transition-all ${selectedTemplate === t
      ...
      <div className="sm:hidden flex gap-1 px-4 py-2 border-b border-slate-800 overflow-x-auto bg-slate-950">
        {TEMPLATES.map((t) => (
          <button
            key={t}
            onClick={() => setSelectedTemplate(t)}
            className={`text-xs px-3 py-1.5 rounded-lg capitalize font-medium transition-all whitespace-nowrap ${selectedTemplate === t ? "bg-teal-500 text-white" : "bg-slate-800 text-slate-400"
              }`}
          >
            {t}
          </button>
